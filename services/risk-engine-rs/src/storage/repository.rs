...
impl RiskRepository {
    ...
    pub async fn load_latest_snapshot(&self, aggregate_id: Uuid) -> Result<Option<SnapshotRecord>, sqlx::Error> {
        self.store.load_snapshot(aggregate_id).await
    }

    pub async fn load_events_since(&self, aggregate_id: Uuid, sequence: i64) -> Result<Vec<EventRecord>, sqlx::Error> {
        self.store.load_events(aggregate_id, sequence).await
    }

    pub async fn rebuild_state<S, F>(&self, aggregate_id: Uuid, initial_state: S, rebuilder: F) -> Result<S, sqlx::Error>
    where
