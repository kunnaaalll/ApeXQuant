...
use apex_protos::execution::{SubmitOrderRequest, NewOrder};
    ...
    pub async fn submit_order(&mut self, order: &ExecutionOrder) -> Result<ExecutionResult, String> {
        let request = tonic::Request::new(SubmitOrderRequest {
            ...
            order: Some(NewOrder {
                ...
                symbol: Some(Symbol { code: order.symbol.clone(), exchange: "".into(), asset_class: 0, description: "".into() }),
                order_type: 1, // MARKET
                side: if order.is_buy { 1 } else { 2 },
                volume: Some(Volume { units: order.quantity.to_string(), lot_size: "".into(), fractional: true }),
                limit_price: None,
                stop_price: None,
                stop_loss: None,
                take_profit: None,
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| format!("ExecutionEngine::SubmitOrder failed for {}: {}", order.symbol, e))?;
