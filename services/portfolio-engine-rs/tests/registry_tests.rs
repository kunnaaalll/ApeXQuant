...
fn test_registry_concurrency() {
    ...
    registry.dispatch(PortfolioEvent::Deposit {
    ...
    for _ in 0..10 {
        ...
        let handle = thread::spawn(move || {
            for _ in 0..100 {
                ...
                registry_clone.dispatch(PortfolioEvent::PositionOpened {
                    position_id: pos_id,
                    margin_used: Decimal::from(10),
                    exposure: Decimal::from(100),
                }).unwrap();

                registry_clone.dispatch(PortfolioEvent::PnlUpdate {
                    position_id: pos_id,
                    pnl_delta: Decimal::from(5),
                }).unwrap();
                
                registry_clone.dispatch(PortfolioEvent::PositionClosed {
                    position_id: pos_id,
