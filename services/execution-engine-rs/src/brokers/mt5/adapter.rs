...
impl Mt5Adapter {
    ...
    pub async fn connect(&self) -> Result<(), BrokerError> {
        ...
        let res = self.http_client.post(&url).send().await.map_err(|e| {
...
impl BrokerAdapter for Mt5Adapter {
    async fn get_account(&self) -> Result<AccountInfo, BrokerError> {
        ...
        let account: Mt5Account = res.json().await.map_err(|e| {
            BrokerError::InvalidMessage(e.to_string())
        })?;
    ...
    async fn get_symbol(&self, symbol: &str) -> Result<SymbolInfo, BrokerError> {
        ...
        let sym: Mt5Symbol = res.json().await.map_err(|e| {
            BrokerError::InvalidMessage(e.to_string())
        })?;
    ...
    async fn get_positions(&self) -> Result<Vec<OpenPosition>, BrokerError> {
        ...
        let positions: Vec<Mt5Position> = res.json().await.map_err(|e| {
            BrokerError::InvalidMessage(e.to_string())
        })?;
    ...
    async fn get_orders(&self) -> Result<Vec<PendingOrder>, BrokerError> {
        ...
        let orders: Vec<Mt5Order> = res.json().await.map_err(|e| {
            BrokerError::InvalidMessage(e.to_string())
        })?;
    ...
    async fn submit_order(&self, req: OrderSubmitRequest) -> Result<OrderSubmitResponse, BrokerError> {
        let url = format!("{}/orders", self.bridge_url);
        let res = self.http_client.post(&url).json(&req).send().await.map_err(|e| {
            BrokerError::ConnectionFailure(e.to_string())
        })?;
        ...
        let resp: SubmitResp = res.json().await.map_err(|e| {
            BrokerError::InvalidMessage(e.to_string())
        })?;
    ...
    async fn modify_order(&self, req: OrderModifyRequest) -> Result<OrderModifyResponse, BrokerError> {
        let url = format!("{}/orders/{}", self.bridge_url, req.order_id);
        let res = self.http_client.put(&url).json(&req).send().await.map_err(|e| {
            BrokerError::ConnectionFailure(e.to_string())
        })?;

        if !res.status().is_success() {
            return Err(BrokerError::OrderModificationFailed(format!("HTTP {}", res.status())));
        }

        Ok(OrderModifyResponse { success: true })
    }

    async fn cancel_order(&self, req: OrderCancelRequest) -> Result<OrderCancelResponse, BrokerError> {
        let url = format!("{}/orders/{}", self.bridge_url, req.order_id);
        let res = self.http_client.delete(&url).send().await.map_err(|e| {
            BrokerError::ConnectionFailure(e.to_string())
        })?;

        if !res.status().is_success() {
            return Err(BrokerError::OrderCancellationFailed(format!("HTTP {}", res.status())));
        }

        Ok(OrderCancelResponse { success: true })
    }

    async fn close_position(&self, req: ClosePositionRequest) -> Result<ClosePositionResponse, BrokerError> {
        ...
        if !res.status().is_success() {
            return Err(BrokerError::PositionCloseFailed(format!("HTTP {}", res.status())));
        }
