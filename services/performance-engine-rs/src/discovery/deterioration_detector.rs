...
impl DeteriorationDetector {
    ...
    pub fn detect(
        ...
        if current_drawdown >= self.max_drawdown_threshold
            || expectancy <= self.min_expectancy_threshold
        {
            return DeteriorationState::Collapse;
        }

        // Warning state logic
        if current_drawdown >= self.max_drawdown_threshold * rust_decimal_macros::dec!(0.8)
            || expectancy <= self.min_expectancy_threshold * rust_decimal_macros::dec!(1.5)
        {
            return DeteriorationState::Critical;
        }

        if current_drawdown >= self.max_drawdown_threshold * rust_decimal_macros::dec!(0.5)
            || stability < rust_decimal_macros::dec!(0.5)
        {
            return DeteriorationState::Warning;
        }

        if stability < rust_decimal_macros::dec!(0.8)
            || expectancy < self.min_expectancy_threshold * rust_decimal_macros::dec!(3.0)
        {
