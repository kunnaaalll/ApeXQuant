...
impl ReinforcementEngine {
    ...
    pub fn process_outcome(&self, outcome: &TradeOutcome, current_weight: Decimal) -> StrategyWeightUpdate {
        ...
        if outcome.profit_loss > Decimal::ZERO {
            ...
            let safe_reward = if reward > Decimal::new(5, 2) { Decimal::new(5, 2) } else { reward }; // Cap at 0.05
            new_weight += safe_reward;
        } else {
            // Penalty: decrease weight based on loss
            let penalty = (outcome.profit_loss.abs() / Decimal::new(500, 0)) + (outcome.max_drawdown / Decimal::new(100, 0));
            let safe_penalty = if penalty > Decimal::new(1, 1) { Decimal::new(1, 1) } else { penalty }; // Cap at 0.1
