...
use backtester::ai_engine_integration::build_ai_engine;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let symbols = vec![
        "EURUSD", "USDJPY", "GBPUSD", "AUDUSD", "USDCAD", "USDCHF", "NZDUSD", "EURGBP", "EURJPY",
        "GBPJPY", "XAUUSD", "BTCUSD", "US30",
    ];

    info!(
        "Loading historical tick databases for symbols: {:?}",
        symbols
    );
    ...
    info!(
        "Successfully replayed {} market ticks in {:?}",
        count,
    ...
    info!(
        "Optimization finished. Best stop loss: {} ticks, Best take profit: {} ticks",
        opt_result.best_stop_loss_ticks, opt_result.best_take_profit_ticks
    );
    ...
    info!(
        "Robustness Validation: PASS (Score: {}%)",
        robustness.breakdown_score * Decimal::new(100, 0)
    );
    ...
    let windows_data: Vec<_> = windows
        .into_iter()
        .map(|w| backtester::walk_forward::WalkForwardWindowData {
            window: w,
            is_stats: backtester::walk_forward::WindowStats {
                total_trades: 100,
                winning_trades: 60,
                gross_profit: Decimal::new(1000, 0),
                gross_loss: Decimal::new(500, 0),
                max_drawdown: Decimal::new(200, 0),
                net_profit: Decimal::new(500, 0),
            },
            oos_stats: backtester::walk_forward::WindowStats {
                total_trades: 50,
                winning_trades: 28,
                gross_profit: Decimal::new(400, 0),
                gross_loss: Decimal::new(200, 0),
                max_drawdown: Decimal::new(100, 0),
                net_profit: Decimal::new(200, 0),
            },
        })
        .collect();
    let wf_result = WalkForwardEngine::evaluate(&windows_data)?;
    info!(
        "Walk-Forward validation: PASS (Regime stability index: {})",
        wf_result.passes_validation
    );
    ...
    let trades = vec![
        backtester::overfitting::OATrade {
            pnl: Decimal::new(150, 2),
        },
        backtester::overfitting::OATrade {
            pnl: Decimal::new(-50, 2),
        },
        backtester::overfitting::OATrade {
            pnl: Decimal::new(300, 2),
        },
        backtester::overfitting::OATrade {
            pnl: Decimal::new(-100, 2),
        },
        backtester::overfitting::OATrade {
            pnl: Decimal::new(200, 2),
        },
    ];
    let overfitting =
        OverfittingAnalyzer::analyze(&param_points, &trades, Decimal::new(15, 1), &[], 42)?;
    info!("Overfitting status: {:?}", overfitting.severity);

    // ── AI Engine Integration ──────────────────────────────────────────────
    info!(
        "Initialising AI Engine integration (LocalAIEngine / RemoteAIEngine via AI_ENGINE_URL)..."
    );
    let ai_engine = build_ai_engine();
    let research_job = ai_engine.request_research_job("EURUSD");
    info!(
        "AI Engine research job created: id={} description='{}'",
        research_job.id, research_job.description
    );
    let param_sets = ai_engine.request_parameter_search(research_job.id);
    info!(
        "AI Engine parameter search returned {} candidate sets",
        param_sets.len()
    );
    if let Some(best_params) = param_sets.first() {
        let blueprint = ai_engine.request_strategy_generation(best_params);
        info!(
            "AI Engine generated blueprint: id={} family={:?}",
            blueprint.id, blueprint.family
        );
    }
    let seg_id = uuid::Uuid::new_v4();
    let features = ai_engine.request_feature_discovery(seg_id);
    info!(
        "AI Engine discovered {} features for segment {}",
        features.len(),
        seg_id
    );
    ...
    info!(
        "Optimization score: best_stop_loss={} ticks, best_take_profit={} ticks, OOS Efficiency={}",
        opt_result.best_stop_loss_ticks,
        opt_result.best_take_profit_ticks,
        wf_result.mean_oos_expectancy,
    );
    info!("Overfitting severity: {:?}", overfitting.severity);
    info!("============================================================");
