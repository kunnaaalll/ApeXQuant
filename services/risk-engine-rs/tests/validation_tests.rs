...
async fn test_determinism_canonical_iterations() {
    ...
    let result = validator
        .validate()
        .expect("Determinism validation must not panic");
...
async fn test_replay_correctness() {
    ...
    let result = validator
        .validate()
        .expect("Replay validation must not panic");
...
async fn test_benchmark_thresholds() {
    ...
    let result = validator
        .validate()
        .expect("Benchmark validation must not panic");
...
async fn test_certification_transitions() {
    ...
    assert_eq!(
        res3.reasons[0],
        "Cannot transition from Rejected to Certified directly"
    );
    ...
    assert!(res_fail
        .reasons
        .contains(&"Agreement 99 is not > 99%".to_string()));
}
...
async fn test_stress_scenarios() {
    ...
    let result = validator
        .validate()
        .expect("Stress validation must not panic");
