use crate::health::health_score::{
    HealthContribution, HealthEvent, PortfolioHealth, PortfolioHealthBreakdown,
    PortfolioHealthState,
};
use rust_decimal::Decimal;

fn default_breakdown() -> PortfolioHealthBreakdown {
    let contrib = HealthContribution {
        weight: Decimal::ZERO,
...
fn test_health_state_boundaries() {
    assert_eq!(
        PortfolioHealth::determine_state(100),
        PortfolioHealthState::Excellent
    );
    assert_eq!(
        PortfolioHealth::determine_state(90),
        PortfolioHealthState::Excellent
    );
    assert_eq!(
        PortfolioHealth::determine_state(89),
        PortfolioHealthState::Healthy
    );
    assert_eq!(
        PortfolioHealth::determine_state(75),
        PortfolioHealthState::Healthy
    );
    assert_eq!(
        PortfolioHealth::determine_state(74),
        PortfolioHealthState::Normal
    );
    assert_eq!(
        PortfolioHealth::determine_state(50),
        PortfolioHealthState::Normal
    );
    assert_eq!(
        PortfolioHealth::determine_state(49),
        PortfolioHealthState::Weak
    );
    assert_eq!(
        PortfolioHealth::determine_state(25),
        PortfolioHealthState::Weak
    );
    assert_eq!(
        PortfolioHealth::determine_state(24),
        PortfolioHealthState::Critical
    );
    assert_eq!(
        PortfolioHealth::determine_state(0),
        PortfolioHealthState::Critical
    );
}
