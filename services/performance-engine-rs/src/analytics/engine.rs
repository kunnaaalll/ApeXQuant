...
impl AnalyticsEngine {
    ...
    pub fn compute(strategy_id: &str, trades: &[ClosedTradeRecord]) -> StrategyAnalyticsResult {
        if trades.is_empty() {
        ...
        let (max_consecutive_wins, max_consecutive_losses) =
            Self::compute_streaks(&trades.iter().map(|t| t.is_win).collect::<Vec<_>>());
        ...
        let calmar_ratio = if max_drawdown.is_zero() {
            ...
            annualised_r / max_drawdown
        };
        let omega_ratio = Self::compute_omega(&r_outcomes);
        let burke_ratio = Self::compute_burke(&r_outcomes, &equity_curve);
        let sterling_ratio = if average_drawdown.is_zero() {
            Decimal::ZERO
        } else {
            annualised_r / average_drawdown
        };
        let mar_ratio = calmar_ratio;

        // ── 12. Edge score ────────────────────────────────────────────────────
        let edge_assessment =
            EdgeCalculator::calculate(expectancy, win_rate, average_rr, trade_count);
    ...
    fn compute_burke(r: &[Decimal], equity: &[Decimal]) -> Decimal {
        ...
        let sqrt_sq =
            Decimal::try_from(sum_sq.to_f64().unwrap_or(0.0).sqrt()).unwrap_or(dec!(0.001));
        if sqrt_sq.is_zero() {
            return Decimal::ZERO;
        }
        mean / sqrt_sq
...
mod tests {
    ...
    fn make_trade(r: f64, is_win: bool) -> ClosedTradeRecord {
        ...
        ClosedTradeRecord {
            ...
            gross_profit: if is_win {
                r_dec * dec!(100)
            } else {
                Decimal::ZERO
            },
            gross_loss: if !is_win {
                (r_dec * dec!(100)).abs()
            } else {
                Decimal::ZERO
            },
    ...
    fn test_mixed_trades_expectancy() {
        ...
        for _ in 0..60 {
            trades.push(make_trade(1.0, true));
        }
        for _ in 0..40 {
            trades.push(make_trade(-1.0, false));
        }
        let result = AnalyticsEngine::compute("s1", &trades);
    ...
    fn test_sharpe_is_deterministic() {
        let trades: Vec<ClosedTradeRecord> = (0..30)
            .enumerate()
            .map(|(i, _)| {
                let is_win = i % 3 != 2;
