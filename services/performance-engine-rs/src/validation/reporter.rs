...
impl CertificationReporter {
    pub fn generate_markdown_report(
        ...
        writeln!(
            file,
            "- **Parity Achieved**: {}",
            if result.passes_parity { "✅" } else { "❌" }
        )?;
        writeln!(
            file,
            "- **Determinism Verified**: {}",
            if result.passes_determinism {
                "✅"
            } else {
                "❌"
            }
        ...
        writeln!(
            file,
            "- **Replay Verified**: {}",
            if result.passes_replay { "✅" } else { "❌" }
        )?;
        writeln!(
            file,
            "- **Monte Carlo Tolerated**: {}",
            if result.passes_monte_carlo {
                "✅"
            } else {
                "❌"
            }
