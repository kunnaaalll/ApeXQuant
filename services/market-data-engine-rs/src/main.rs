...
use market_data_engine_rs::mt5_stream::{Mt5TickStream, Mt5Config};
...
use market_data_engine_rs::storage::{MarketDataStore, TickRecord, CandleRecord as DbCandleRecord};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let symbols: Vec<String> = allowed_symbols.split(',').map(|s| s.trim().to_uppercase()).collect();
    ...
    for symbol in symbols {
        ...
        tokio::spawn(async move {
            ...
            let mut stream: Box<dyn TickStream> = if symbol.contains("BTC") || symbol.contains("ETH") || symbol.contains("USDT") {
                Box::new(BinanceTickStream::new_spot(symbol.clone()))
            } else {
                let mut config = Mt5Config::default();
                config.endpoint = env::var("MT5_BRIDGE_ENDPOINT").unwrap_or_else(|_| "127.0.0.1:5555".to_string());
            ...
            while let Some(raw_tick) = stream.next_tick().await {
                ...
                let missing = if raw_tick.sequence > expected_sequence { raw_tick.sequence - expected_sequence } else { 0 };
                
                if let Some(gap_event) = gap_detector.process_tick(raw_tick.timestamp, missing, is_duplicate) {
